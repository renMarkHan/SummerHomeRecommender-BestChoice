...
from user import create_user, User, get_importance
import database
...
def get_recommendations_menu():
    ...
    try:
        ...
        we_afford = get_importance("On a scale from 0 to 10, how important is your budget when choosing a property? ")
        we_pref_env = get_importance("On a scale from 0 to 10, how important is the environment or location to you? ")
        ...
        # Get matches
        matches = current_user.match_properties(we_afford,we_pref_env).head(5)
        print(matches)

        print("We’ve identified the top 5 properties that best match your preferences : \n ")
        top_matches = matches[["property_id", "location", "type", "nightly_price", "features","tags", "match_score"]]

        for i, row in top_matches.iterrows():
            print(f"{i+1}. 🏠 Property ID: {row['property_id']}")
            print(f"📍 Location: {row['location']}")
            print(f"🏡 Type: {row['type']}")
            print(f"💰 Nightly Price: ${row['nightly_price']}")
            print(f"🛠️ Features: {', '.join(row['features']) if isinstance(row['features'], list) else row['features']}")
            print(f"🏷️ Tags: {', '.join(row['tags']) if isinstance(row['tags'], list) else row['tags']}")
