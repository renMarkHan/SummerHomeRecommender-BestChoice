...
const API_CONFIG = {
    ...
    ENDPOINTS: {
        ...
        HEALTH: '/health',
        ROOT: '/',
        FILTER_PROPERTIES: '/api/filter_properties',
...
/**
 * Filter properties using vectorized backend filtering
 * @param {Object} filterParams - Filter parameters
 * @returns {Promise} - Returns Promise with filtered properties
 */
async function filterProperties(filterParams) {
    try {
        console.log('Applying vectorized filters:', filterParams);
        
        const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.FILTER_PROPERTIES}`;
        const response = await makeRequestWithRetry(url, {
            method: 'POST',
            body: filterParams,
            timeout: 30000
        ...
        console.log('Vectorized filtering completed:', response.data);
        return response.data;
        
    } catch (error) {
        console.error('Failed to apply vectorized filters:', error);
        throw error;
    }
}
 ...
 * @returns {Promise} - Returns Promise with filter options
 */
async function getFilterOptions() {
    try {
        console.log('Fetching filter options...');
        
        const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.FILTER_OPTIONS}`;
        const response = await makeRequest(url, {
            timeout: 15000
        });
        
        console.log('Filter options retrieved:', response.data);
        return response.data;
